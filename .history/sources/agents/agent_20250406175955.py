...
# 4. After Gemini, include additional agents as needed to complete the plan.
    ...
    def extract_answer(self, text, user_prompt):
        response: ChatResponse = chat(model='llama3.2:1b', messages=[
            {
                'role': 'user',
                'content': f"""
    You are a helpful assistant. The user gave a prompt and a response generated by an LLM.
    Your job is to extract only the part of the LLM's response that is **relevant to the user's question**.
    ...
    1. Extract only the portion of the response that is relevant, helpful, or similar to the user's query.
    2. If nothing is relevant, return an empty string.
    3. Do not return anything else — no explanations, no formatting, just the cleaned relevant answer.

    Only return the extracted answer.
    """
            }
        ])
            ...
            if blocks != None:
                ...
                for block in blocks:
                    try:
                        block = block.split('\n')
                        if 'mkdir' in block:
                            self.block_reference = block.split('')[-1]
                        if '' in block:
                            block.remove('')
                    except:
                        continue
                    pretty_print(f'Block: {block}')
