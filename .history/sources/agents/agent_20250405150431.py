...
"""
  ...
  "plan": [
    ...
    },
    {
      ...
      "id": "3",
      "need": "",
      "task": "{{task_description_3}}"
    },
    {
      "agent": "Coder",
      "id": "4",
      "need": "2,3",
      "task": "{{task_description_4}}"
    }
  ]
}
    ...
    def format_llm_request(self, unformatted_llm_response):

        response: ChatResponse = chat(model='llama3.2:1b', messages=[
    {
        'role': 'user',
        'content': f"""
Your task is to return a JSON-formatted response following this structure:
{self.json_format}

Instructions:

1. If the input object is not in valid JSON format, fix it and convert it to match the format above.
5. Do not include any explanation or prose — only output the final JSON.
6. The list of agents available to you are as follows : {self.agent_list}

Now, process the following input and return only the corrected JSON plan:
