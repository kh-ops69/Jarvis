...
class PlannerAgent(Agent):
    def __init__(self, name, prompt_path, provider, verbose=False, browser=None):
        ...
        self.lang_interpreters = ["python3", "gcc", "g++", "clang", "clang++"]
    ...
    def process(self, prompt, speech_module) -> str:
        ...
        agents = self.parse_agents(answer)
        pretty_print(agents)
        ...
        for agent, (task_name, task) in zip(agents, agents_tasks):
            ...
            if len(tasks_completed)>0 and tasks_completed[0] == 'gemini':
            ...
            for interpreter in self.lang_interpreters:
                if interpreter in task_name.lower():
                    try:
